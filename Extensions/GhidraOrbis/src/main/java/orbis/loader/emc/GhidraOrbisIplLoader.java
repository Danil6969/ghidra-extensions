...
public class GhidraOrbisIplLoader extends AbstractProgramLoader {
	...
	protected List<Loaded<Program>> loadProgram(ByteProvider provider, String programName,
			...
			MessageLog log, Object consumer, TaskMonitor monitor) throws IOException, CancelledException {
		...
		return prog != null ? List.of(new Loaded<>(prog, programName, programFolderPath)) : Collections.emptyList();
	}
	...
	protected void loadProgramInto(ByteProvider provider, LoadSpec loadSpec,
		...
		try {
			...
			if (header.isEAP()) {
				...
				ElfHeader elf = new ElfHeader(bp, null);
